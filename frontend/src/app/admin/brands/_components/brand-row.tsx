...
export default function BrandRow({
  ...
  const handleSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await brandService.update(brand.id, form);
  ...
  const handleUploadLogo = async (file: File) => {
    setIsUploading(true);
    try {
      await brandService.uploadLogo(brand.id, file);
    ...
    >
      ...
      <td className="py-2.5 px-4">
        <div className="relative group">
          <div className="relative w-9 h-9 rounded-lg bg-gray-50 overflow-hidden border border-gray-100">
            {brand.logoUrl ? (
              <img
                src={brand.logoUrl}
                alt={brand.name}
      ...
      <td className="py-2.5 px-4">
        {isEditing ? (
          ...
          <span className="text-gray-400 font-mono text-xs">{brand.slug}</span>
        )}
      </td>
